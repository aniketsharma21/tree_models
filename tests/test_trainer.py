...
from tree_models.models.trainer import (
    ModelTrainer, TrainingConfig, TrainingResults, train_model
)
...
class TestModelTrainer:
    ...
    def test_validate_training_inputs_valid(self, trainer, sample_data):
        ...
        trainer._validate_training_inputs(
            X, y, X_valid, y_valid, sample_weight, weight_valid
        )
    ...
    def test_validate_training_inputs_invalid(self, trainer, sample_data):
        ...
        with pytest.raises(Exception):
            trainer._validate_training_inputs(
                pd.DataFrame(), pd.Series(dtype=float), None, None, None, None
            )
        
        # Mismatched lengths
        with pytest.raises(Exception):
            trainer._validate_training_inputs(
                X, y.iloc[:-1], None, None, None, None
            )
        ...
        with pytest.raises(Exception):
            trainer._validate_training_inputs(
                X, y, None, None, bad_weights, None
            )
    ...
    def test_train_xgboost_mock(self, mock_xgb, trainer, sample_data):
        ...
        results = trainer._train_xgboost(
            mock_model, model_config, X, y, None, None,
            sample_weight, None, {'n_estimators': 100}, training_config
        )
