...
class TestEndToEndPipeline:
    ...
    def sample_dataset(self):
        ...
        df.loc[list(outlier_indices), "credit_score"] = np.random.uniform(300, 350, 10)
    ...
    def data_config(self):
        ...
        return DataConfig(
            source=DataSourceConfig(file_format="csv"),
    ...
    def test_data_validation_pipeline(self, sample_dataset):
        ...
        config = ValidationConfig(
            check_missing_values=True, check_outliers=True, check_distributions=True, generate_report=False
        )

        # Run validation
        results = validator.validate_dataset(sample_dataset, target_column="approved", config=config)
    ...
    def test_preprocessing_pipeline(self, sample_dataset, data_config):
        ...
        column_configs = {
            "income": ColumnConfig(missing_strategy="median", scaling_strategy="standard", outlier_method="clip"),
            "education": ColumnConfig(
                encoding_strategy="onehot", missing_strategy="constant", missing_constant="Unknown"
            ),
            "city": ColumnConfig(encoding_strategy="target", missing_strategy="most_frequent"),
        }
    ...
    def test_feature_engineering_pipeline(self, sample_dataset):
        ...
        # Engineer features
        results = engineer.engineer_features(sample_dataset, config=config, target_column="approved")
    ...
    def test_training_pipeline_mock(self, mock_xgb, sample_dataset, data_config):
        ...
        training_config = TrainingConfig(
            enable_early_stopping=False, enable_cross_validation=False, verbose=False  # Simplify for testing
        )
        ...
        # Train model
        results = trainer.train_model(model_config, X_train, y_train, X_test, y_test, training_config=training_config)
    ...
    def test_complete_pipeline_integration(self, sample_dataset):
        ...
        validation_results = validator.validate_dataset(
            sample_dataset, target_column="approved", config=ValidationConfig(generate_report=False)
        )
        ...
        eval_results = evaluator.evaluate_model(mock_model, X_final, y, config=EvaluationConfig(include_plots=False))
...
class TestPipelineErrorHandling:
    ...
    def test_mismatched_data_handling(self):
        ...
        train_data = pd.DataFrame(
            {"feature1": [1, 2, 3, 4, 5], "feature2": ["a", "b", "c", "d", "e"], "target": [0, 1, 0, 1, 0]}
        )

        # Test data with different features
        test_data = pd.DataFrame({"feature1": [6, 7, 8], "feature3": ["f", "g", "h"]})  # Different feature name
