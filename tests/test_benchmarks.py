...
class TestPerformanceBenchmarks:
    ...
    def benchmark_datasets(self, request):
        ...
        # Target variable
        data['target'] = np.random.binomial(1, 0.3, n_samples)
        ...
        for col in df.columns:
            if col == 'target':
                ...
                config = ColumnConfig(
                    ...
                    encoding_strategy='label'  # Faster than onehot for benchmarking
                )
            else:
                config = ColumnConfig(
    ...
    def test_evaluator_performance(self, benchmark_datasets):
        ...
        predictions = np.where(y == 1, 
                              np.minimum(base_predictions + 0.3, 0.95),
                              np.maximum(base_predictions - 0.3, 0.05))
...
class TestMemoryEfficiency:
    ...
    def test_large_dataset_memory_usage(self):
        ...
        for i in range(0, n_samples, chunk_size):
            ...
            chunk_data = {
                f'feature_{j}': np.random.randn(current_chunk_size) 
                for j in range(n_features)
            }
            chunk_data['target'] = np.random.randint(0, 2, current_chunk_size)
        ...
        assert validation_overhead < data_size_mb * 2, f"Validation memory overhead too high: {validation_overhead:.2f}MB"
...
class TestScalabilityLimits:
    ...
    def test_wide_dataset_performance(self):
        ...
        validator = DataValidator()
        config = ValidationConfig(
            check_correlations=False,  # Would be very expensive
            generate_report=False
        )
...
class TestResourceMonitoring:
    ...
    def test_memory_leak_detection(self):
        ...
        for i in range(10):
            ...
            df = pd.DataFrame({
                f'feature_{j}': np.random.randn(1000) 
                for j in range(10)
            })
