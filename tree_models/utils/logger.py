...
class PerformanceLoggerAdapter(logging.LoggerAdapter):
    ...
    def stop_timer(self, name: str) -> float:
        ...
        # Include duration in the message itself so captured records contain timing
        self.info(
            f"Timer '{name}' completed | Duration: {duration:.3f}s",
            extra={"context": {"timer_action": "stop", "timer_name": name}, "duration": duration},
        )
...
class TreeModelsLogger:
    ...
    @classmethod
    def get_logger(cls, name: str, with_performance: bool = False) -> Union[logging.Logger, PerformanceLoggerAdapter]:
...
def configure_logging(
    level: Union[str, int] = logging.INFO, log_file: Optional[Union[str, Path]] = None, **kwargs: Any
) -> None:
    """Configure package-wide logging settings.
