...
class PreprocessingConfig:
    ...
    categorical_encoding: Dict[str, str] = field(default_factory=lambda: {
    ...
    def __post_init__(self) -> None:
        ...
        validate_parameter("handle_unknown_categories", self.handle_unknown_categories, valid_values=valid_unknown_handling)
...
class SampleWeightConfig:
    ...
    def __post_init__(self) -> None:
        ...
        validate_parameter("weight_computation_method", self.weight_computation_method, valid_values=valid_weight_methods)
...
class DataConfig:
    ...
    def _validate_configuration_consistency(self) -> None:
        ...
        if (self.features.exclude_columns and 
            self.features.target_column in self.features.exclude_columns):
            raise ConfigurationError("Target column cannot be in excluded columns")
        
        # Check sample weights column consistency
        if (self.sample_weights.use_sample_weights and 
            self.sample_weights.weight_column and
            self.features.exclude_columns and
            self.sample_weights.weight_column in self.features.exclude_columns):
        ...
        if (self.splitting.time_series_split and 
            not self.splitting.time_column):
            raise ConfigurationError("time_column must be specified for time series splitting")
        
        # Check feature selection consistency
        if (self.features.enable_feature_selection and 
            self.features.n_features_to_select and
            self.features.feature_columns and
            self.features.n_features_to_select > len(self.features.feature_columns)):
...
def create_advanced_data_config(
    ...
    **kwargs: Any
    ...
    source_config = DataSourceConfig(
        train_path=train_path,
        **{k: v for k, v in kwargs.items() if k.startswith('source_')}
    )
    ...
    preprocessing_config = PreprocessingConfig(
        ...
        **{k: v for k, v in kwargs.items() if k.startswith('preprocessing_')}
    )
    
    sample_weights_config = SampleWeightConfig(
        balance_classes=balance_classes,
        **{k: v for k, v in kwargs.items() if k.startswith('weights_')}
    )
    
    validation_config = ValidationConfig(
        generate_report=enable_validation,
        **{k: v for k, v in kwargs.items() if k.startswith('validation_')}
    )
    
    # Filter out prefixed kwargs
    main_kwargs = {k: v for k, v in kwargs.items() 
                  if not any(k.startswith(prefix) for prefix in 
                           ['source_', 'features_', 'preprocessing_', 'weights_', 'validation_'])}
