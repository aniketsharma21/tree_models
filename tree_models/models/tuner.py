...
class ScoringConfig:
    ...
    def __post_init__(self) -> None:
        ...
        validate_parameter("cv_strategy", self.cv_strategy, 
                         valid_values=["stratified", "kfold", "time_series"])
...
class EnhancedCustomScorer:
    ...
    def __call__(self, estimator: Any, X: pd.DataFrame, y: pd.Series) -> float:
        ...
        try:
            with timed_operation(f"scoring_{self.scoring_function}", timeout=self.timeout) as timing:
                ...
                score = self._compute_score_safely(
                    scorer_func, y, y_pred, use_proba, supports_weights
                )
    ...
    def _compute_score_safely(
        ...
        binary_scorers = {"accuracy", "precision", "recall", "f1", "matthews_corrcoef", 
...
class OptunaHyperparameterTuner(BaseHyperparameterTuner):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        validate_parameter("pruner", pruner, 
                         valid_values=["median", "successive_halving", "hyperband", "nop"])
    ...
    def _create_study(self) -> optuna.Study:
        ...
        direction = optuna.study.StudyDirection.MAXIMIZE if self.scoring_config.direction == "maximize" else optuna.study.StudyDirection.MINIMIZE
    ...
    def _evaluate_params_with_cv(
        ...
        scorer = EnhancedCustomScorer(
            self.scoring_config.scoring_function,
            self._sample_weight,
            self.scoring_config.timeout_per_trial
        )
        ...
        try:
            ...
            handle_and_reraise(
                e, ModelTrainingError,
                f"Cross-validation failed for params: {params}",
                error_code="CV_FAILED"
            )
    ...
    def _evaluate_additional_metrics(
        ...
        if (self.scoring_config.evaluation_sample_size and 
            len(X_eval) > self.scoring_config.evaluation_sample_size):
            
            indices = np.random.choice(
                len(X_eval), 
                self.scoring_config.evaluation_sample_size, 
                replace=False
            )
    ...
    def save_study(self, filepath: Union[str, Path]) -> None:
        ...
        try:
            ...
            handle_and_reraise(
                ...
                f"Failed to save study to {filepath}",
    ...
    def load_study(self, filepath: Union[str, Path]) -> None:
        ...
        try:
            ...
            handle_and_reraise(
                e, FileOperationError,
                f"Failed to load study from {filepath}",
                error_code="STUDY_LOAD_FAILED"
            )
