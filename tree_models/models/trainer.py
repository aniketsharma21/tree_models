...
class ModelTrainer:
    ...
    def __init__(
        self, random_state: int = 42, enable_logging: bool = True, checkpoint_dir: Optional[Union[str, Path]] = None
    ) -> None:
        ...
        try:
            with timed_operation("model_training") as timing:
                ...
                if training_config.validate_input_data:
                    self._validate_training_inputs(X_train, y_train, X_valid, y_valid, sample_weight, validation_weight)
    ...
    def _create_validation_split(
        self, X: pd.DataFrame, y: pd.Series, sample_weight: Optional[np.ndarray], validation_fraction: float
    ...
    def _prepare_training_parameters(
        self, model_config: ModelConfig, training_config: TrainingConfig, has_validation: bool
    ) -> Dict[str, Any]:
    ...
    def _train_xgboost(
        ...
        # Compute metrics (defensive: ensure prediction lengths match labels)
        train_pred = model.predict(dtrain)
        train_pred = np.asarray(train_pred).ravel()

        # Align train predictions with labels if lengths differ
        n_train = min(len(train_pred), len(y_train))
        if len(train_pred) != len(y_train):
            logger.warning(
                "XGBoost returned train predictions with unexpected length (%d), expected %d. Trimming to %d.",
                len(train_pred),
                len(y_train),
                n_train,
            )
            train_pred = train_pred[:n_train]
            y_train_for_metrics = y_train.iloc[:n_train] if isinstance(y_train, pd.Series) else y_train[:n_train]
            sw_train = sample_weight[:n_train] if sample_weight is not None else None
        else:
            y_train_for_metrics = y_train
            sw_train = sample_weight

        train_metrics = self._compute_metrics(y_train_for_metrics, train_pred, sw_train)
        ...
        if X_valid is not None:
            ...
            n_valid = min(len(valid_pred), len(y_valid))
            if len(valid_pred) != len(y_valid):
                logger.warning(
                    "XGBoost returned valid predictions with unexpected length (%d), expected %d. Trimming to %d.",
                    len(valid_pred),
                    len(y_valid),
                    n_valid,
                )
                valid_pred = valid_pred[:n_valid]
                y_valid_for_metrics = y_valid.iloc[:n_valid] if isinstance(y_valid, pd.Series) else y_valid[:n_valid]
                sw_valid = validation_weight[:n_valid] if validation_weight is not None else None
            else:
                y_valid_for_metrics = y_valid
                sw_valid = validation_weight

            validation_metrics = self._compute_metrics(y_valid_for_metrics, valid_pred, sw_valid)

        # Determine best iteration (defensive coercion)
        best_iteration = getattr(model, "best_iteration", None)
        if best_iteration is not None:
            try:
                # Try to coerce to a plain int
                if isinstance(best_iteration, (np.integer,)):
                    best_iteration = int(best_iteration)
                elif not isinstance(best_iteration, int):
                    best_iteration = int(best_iteration)
            except Exception:
                logger.warning(
                    "Could not interpret model.best_iteration value (%r); treating as unknown.", best_iteration
                )
                best_iteration = None
        ...
        if best_iteration is None and evals_result:
            ...
            if "valid" in evals_result:
                ...
                if "error" in eval_metric or "loss" in eval_metric:
                    best_iteration = int(np.argmin(scores))
                else:
                    best_iteration = int(np.argmax(scores))
            else:
                best_iteration = int(len(evals_result["train"][list(evals_result["train"].keys())[0]]) - 1)

        early_stopped = False
        if isinstance(best_iteration, int):
            try:
                early_stopped = best_iteration < (num_round - 1)
            except Exception:
                early_stopped = False
    ...
    def _train_catboost(
        ...
        if X_valid is not None:
            valid_pred = (
                model.predict_proba(X_valid)[:, 1] if hasattr(model, "predict_proba") else model.predict(X_valid)
            )
            validation_metrics = self._compute_metrics(y_valid, valid_pred, validation_weight)
    ...
    def _compute_metrics(
        self, y_true: pd.Series, y_pred: np.ndarray, sample_weight: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
        ...
        try:
            ...
            if unique_values == 2:
                ...
                metrics["precision"] = precision_score(
                    y_true, y_pred_binary, sample_weight=sample_weight, zero_division=0
                )
                ...
                if sample_weight is not None:
                    ...
                    ss_tot = np.average(
                        (y_true - np.average(y_true, weights=sample_weight)) ** 2, weights=sample_weight
                    )
                else:
        ...
        try:
            ...
            if model_config.model_type == "xgboost" and XGBOOST_AVAILABLE:
                ...
                estimator = (
                    ...
                    if y.nunique() == 2
                    else cb.CatBoostRegressor(**params, verbose=False)
                )
            else:
            ...
            cv_splitter = (
                StratifiedKFold(n_splits=training_config.cv_folds, shuffle=True, random_state=self.random_state)
                if y.nunique() == 2
                else None
            )
    ...
    def save_model(self, model: Any, model_path: Union[str, Path], results: Optional[TrainingResults] = None) -> None:
        ...
        try:
            ...
            handle_and_reraise(
                e, ModelTrainingError, f"Failed to save model to {model_path}", error_code="MODEL_SAVE_FAILED"
            )
    ...
    def load_model(self, model_path: Union[str, Path]) -> Tuple[Any, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            handle_and_reraise(
                e, ModelTrainingError, f"Failed to load model from {model_path}", error_code="MODEL_LOAD_FAILED"
            )
...
def train_model(
    ...
    **kwargs: Any,
    ...
    """
    trainer = ModelTrainer()
    return trainer.train_model(model_config, X_train, y_train, X_valid, y_valid, sample_weight, **kwargs)


# Alias for backward compatibility or standard naming
StandardModelTrainer = ModelTrainer

# Export key classes and functions
__all__ = ["TrainingConfig", "TrainingResults", "ModelTrainer", "StandardModelTrainer", "train_model"]
