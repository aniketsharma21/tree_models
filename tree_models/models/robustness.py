...
class RobustnessConfig:
    ...
    def __post_init__(self) -> None:
        ...
        validate_parameter(
            "perturbation_method", self.perturbation_method, valid_values=["gaussian", "uniform", "percentage"]
        )

        # Validate PSI parameters
        validate_parameter("psi_bins", self.psi_bins, min_value=5, max_value=50)
        validate_parameter("psi_method", self.psi_method, valid_values=["quantile", "equal_width"])
...
class SeedRobustnessTester(BaseRobustnessTester):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        super().__init__(test_type="seed_robustness", n_iterations=n_seeds, **kwargs)
        ...
        logger.info("Initialized SeedRobustnessTester:")
    ...
    def _train_single_seed(
        ...
        context = create_error_context(
            seed=seed, model_type=model_trainer.model_type, data_shape=X.shape, has_weights=sample_weight is not None
        )
        ...
        try:
            with timed_operation(f"seed_{seed}_training", timeout=self.config.timeout_per_seed) as timing:
                ...
                if sample_weight is not None:
                    X_train, X_test, y_train, y_test, sw_train, sw_test = train_test_split(
                        X, y, sample_weight, test_size=self.config.test_size, random_state=seed, stratify=y
                    )
                else:
                ...
                # Evaluate model
                metrics = self._evaluate_model_comprehensive(model, X_test, y_test, sw_test)
            ...
            logger.debug(
                f"Seed {seed} completed in {training_duration:.2f}s: {self.scoring_function}={metrics.get(self.scoring_function, 'N/A')}"
            )
    ...
    def _evaluate_model_comprehensive(
        self, model: Any, X_test: pd.DataFrame, y_test: pd.Series, sample_weight: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
        ...
        try:
                    ...
                    if sample_weight is not None and supports_weights:
                        if metric_name in ["precision", "recall", "f1"]:
                            metrics[metric_name] = metric_func(
                                y_test, pred_input, sample_weight=sample_weight, zero_division=0
                            )
                        else:
                            metrics[metric_name] = metric_func(y_test, pred_input, sample_weight=sample_weight)
                    else:
                        if metric_name in ["precision", "recall", "f1"]:
            ...
            return dict.fromkeys(["roc_auc", "average_precision", "accuracy", "precision", "recall", "f1"], np.nan)
    ...
    def test_robustness(
        ...
        **kwargs: Any,
        ...
        """
        logger.info("🛡️ Starting seed robustness testing:")
        ...
        try:
            ...
            for i, seed in enumerate(seeds, 1):
                ...
                try:
                    result = self._train_single_seed(seed, model_trainer, X, y, model_params, sample_weight)
                    ...
                    self.raw_results.append(
                        {
                            ...
                            "metrics": dict.fromkeys(
                                ["roc_auc", "average_precision", "accuracy", "precision", "recall", "f1"], np.nan
                            ),
                        }
                    )
            ...
            logger.info("✅ Seed robustness testing completed:")
            logger.info(f"   Success rate: {successful_runs}/{len(seeds)} ({successful_runs / len(seeds) * 100:.1f}%)")
            logger.info(
                f"   Mean {self.scoring_function}: {analysis['metrics'][self.scoring_function]['mean']:.4f} ± {analysis['metrics'][self.scoring_function]['std']:.4f}"
            )
            ...
            handle_and_reraise(
                ...
                context=create_error_context(
                    n_seeds=self.config.n_seeds, model_type=model_trainer.model_type, data_shape=X.shape
                ),
            )
        ...
        for metric_name, values in all_metrics.items():
            if len(values) >= 2:
                ...
                if len(values) >= 3:
                    ...
                    ci = stats.t.interval(confidence, len(values) - 1, loc=np.mean(values), scale=stats.sem(values))
                    metrics_analysis[metric_name]["ci_lower"] = ci[0]
                    metrics_analysis[metric_name]["ci_upper"] = ci[1]

        # Analyze feature importance stability
        feature_importance_analysis = self._analyze_feature_importance_stability(successful_results, feature_names)
    ...
    def _analyze_feature_importance_stability(
        self, successful_results: List[Dict[str, Any]], feature_names: List[str]
    ...
    def _calculate_stability_metrics(self, analysis: Dict[str, Any]) -> Dict[str, float]:
        ...
        stability_scores = [
            v
            for v in [stability_metrics.get("performance_stability"), stability_metrics.get("feature_stability")]
    ...
    def get_stability_metrics(self) -> Dict[str, float]:
        ...
        if self.stability_metrics_ is None:
            raise RobustnessTestError(
                "No stability metrics available. Run test_robustness() first.", error_code="NO_STABILITY_METRICS"
            )
    ...
    def get_detailed_analysis(self) -> Dict[str, Any]:
        ...
        if not hasattr(self, "test_results_"):
            raise RobustnessTestError(
                "No test results available. Run test_robustness() first.", error_code="NO_TEST_RESULTS"
            )
    ...
    def save_results(self, filepath: Union[str, Path]) -> None:
        ...
        try:
            ...
            handle_and_reraise(
                e, RobustnessTestError, f"Failed to save results to {filepath}", error_code="SAVE_FAILED"
            )
...
def quick_robustness_test(
    ...
    tester = SeedRobustnessTester(n_seeds=n_seeds, scoring_function=scoring_function, config=config)

    # Run robustness test
    detailed_results = tester.test_robustness(
        model_trainer=trainer, X=X, y=y, model_params=best_params, sample_weight=sample_weight
    )
    ...
    return {
        ...
        "seed_results": [
            {"seed": r["seed"], "score": r["metrics"].get(scoring_function, np.nan), "success": r.get("success", False)}
...
def calculate_psi_simple(X_reference: pd.DataFrame, X_comparison: pd.DataFrame, bins: int = 10) -> Dict[str, float]:
    ...
    for column in X_reference.columns:
        if column in X_comparison.columns:
            try:
                psi_value = psi_calculator.calculate_psi(X_reference[column], X_comparison[column])
...
SensitivityAnalyzer = SeedRobustnessTester
