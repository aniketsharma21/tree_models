...
class EvaluationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        validate_parameter(
            "threshold_metric", self.threshold_metric, valid_values=["f1", "precision", "recall", "youden", "profit"]
        )
...
class ModelEvaluator:
    ...
    def evaluate_model(
        ...
        **kwargs: Any,
        ...
        Raises:
            ModelEvaluationError: If model evaluation fails
        """
        ...
        try:
            with timed_operation("model_evaluation") as timing:
                ...
                if config.compute_confidence_intervals:
                    confidence_intervals = self._compute_confidence_intervals(y, predictions, sample_weight, config)
                    results.confidence_intervals = confidence_intervals

                # Business metrics
                if config.compute_business_metrics and task_type == "classification":
                    business_metrics = self._compute_business_metrics(y, predictions, sample_weight, config)
    ...
    def _validate_evaluation_inputs(
        self, model: Any, X: pd.DataFrame, y: pd.Series, sample_weight: Optional[np.ndarray]
    ) -> None:
    ...
    def _evaluate_classification(
        ...
        try:
            ...
            if config.compute_roc_auc and len(np.unique(y_true)) == 2:
                ...
                results.roc_data = {"fpr": fpr, "tpr": tpr, "thresholds": roc_thresholds, "auc": metrics["auc"]}
            ...
            metrics["precision"] = precision_score(
                y_true, y_pred_binary, sample_weight=sample_weight, average="weighted", zero_division=0
            )
            ...
            try:
                ...
                report = classification_report(
                    y_true, y_pred_binary, sample_weight=sample_weight, output_dict=True, zero_division=0
                )
            ...
            if config.compute_calibration and len(np.unique(y_true)) == 2:
                calibration_data = self._compute_calibration_analysis(y_true, y_pred_proba, sample_weight)
    ...
    def _evaluate_regression(
        ...
        try:
            ...
            if mask.any():
                if sample_weight is not None:
                    mape = np.average(
                        np.abs((y_true[mask] - y_pred[mask]) / y_true[mask]) * 100, weights=sample_weight[mask]
                    )
                else:
            ...
            metrics["mean_residual"] = (
                np.average(residuals, weights=sample_weight) if sample_weight is not None else np.mean(residuals)
            )
            metrics["std_residual"] = (
                np.sqrt(np.average(residuals**2, weights=sample_weight))
                if sample_weight is not None
    ...
    def _compute_calibration_analysis(
        self, y_true: pd.Series, y_pred_proba: np.ndarray, sample_weight: Optional[np.ndarray], n_bins: int = 10
    ) -> Dict[str, Any]:
    ...
    def _optimize_classification_threshold(
        self, y_true: pd.Series, y_pred_proba: np.ndarray, sample_weight: Optional[np.ndarray], config: EvaluationConfig
    ) -> Dict[str, Any]:
        ...
        for threshold in thresholds:
            ...
            try:
                ...
                threshold_metrics.append(
                    {"threshold": threshold, "precision": 0, "recall": 0, "f1": 0, "tp": 0, "fp": 0, "tn": 0, "fn": 0}
                )
    ...
    def compare_models(
        self, models_results: Dict[str, EvaluationResults], comparison_metric: str = "auc"
    ) -> Dict[str, Any]:
    ...
    def generate_report(
        self, results: EvaluationResults, output_dir: Union[str, Path], include_plots: bool = True
    ) -> Dict[str, Path]:
        ...
        try:
            ...
            handle_and_reraise(
                e, ModelEvaluationError, "Failed to generate evaluation report", error_code="REPORT_GENERATION_FAILED"
            )

    def _generate_evaluation_plots(self, results: EvaluationResults, output_dir: Path) -> Dict[str, Path]:
    ...
    def _plot_roc_curve(self, roc_data: Dict[str, np.ndarray], output_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            plt.plot(roc_data["fpr"], roc_data["tpr"], linewidth=2, label=f"ROC Curve (AUC = {roc_data['auc']:.4f})")
    ...
    def _plot_pr_curve(self, pr_data: Dict[str, np.ndarray], output_dir: Path) -> Optional[Path]:
        ...
        try:
            plt.figure(figsize=(8, 6))
            plt.plot(
                pr_data["recall"], pr_data["precision"], linewidth=2, label=f"PR Curve (AUC = {pr_data['auc']:.4f})"
            )
...
def evaluate_model(
    model: Any, X: pd.DataFrame, y: pd.Series, sample_weight: Optional[np.ndarray] = None, **kwargs: Any
) -> EvaluationResults:
...
def compare_models(
    ...
    for model_name, model in models_dict.items():
        results[model_name] = evaluator.evaluate_model(model, X, y, sample_weight, model_name=model_name)
...
StandardModelEvaluator = ModelEvaluator
