...
from ..utils.exceptions import DataValidationError, create_error_context, handle_and_reraise, validate_parameter
...
try:
    ...
    from scipy.stats import chi2_contingency, jarque_bera, ks_2samp, kstest, shapiro
    ...
    def __post_init__(self) -> None:
        ...
        for method in self.outlier_methods:
            if method not in valid_outlier_methods:
                raise DataValidationError(f"Unknown outlier method: {method}")
...
class DataValidator:
    ...
    def validate_dataset(
        ...
        Raises:
            DataValidationError: If validation configuration is invalid
        """
    ...
    def _validate_basic_properties(
        self, data: pd.DataFrame, results: ValidationResults, config: ValidationConfig
    ) -> None:
    ...
    def _validate_schema(self, data: pd.DataFrame, results: ValidationResults, config: ValidationConfig) -> None:
    ...
    def _analyze_missing_values(self, data: pd.DataFrame, results: ValidationResults, config: ValidationConfig) -> None:
        ...
        for col in data.columns:
            ...
            missing_stats[col] = {"count": missing_count, "percentage": missing_percentage}

            total_missing += missing_count

            # Flag columns with high missing rates
            if missing_percentage > config.missing_threshold * 100:
                results.validation_errors.append(f"Column '{col}' has high missing rate: {missing_percentage:.1f}%")
        ...
        if overall_missing_rate > 30:
            results.validation_errors.append(f"Dataset has very high overall missing rate: {overall_missing_rate:.1f}%")
        elif overall_missing_rate > 10:
            results.validation_warnings.append(f"Dataset has high overall missing rate: {overall_missing_rate:.1f}%")
        ...
        for method in config.outlier_methods:
            try:
                ...
                if method == "iqr":
                    ...
                    method_results = self._detect_outliers_modified_zscore(
                        data[numeric_columns], config.outlier_threshold
                    )
                elif method == "isolation" and SKLEARN_AVAILABLE:
                    method_results = self._detect_outliers_isolation(
                        data[numeric_columns], config.isolation_contamination
                    )
                elif method == "elliptic" and SKLEARN_AVAILABLE:
                    method_results = self._detect_outliers_elliptic(
                        data[numeric_columns], config.isolation_contamination
                    )
    ...
    def _detect_outliers_zscore(self, data: pd.DataFrame, threshold: float = 3.0) -> Dict[str, Any]:
        ...
        for col in data.columns:
            ...
            z_scores = (
                ...
                if SCIPY_AVAILABLE
                else np.abs((col_data - col_data.mean()) / col_data.std())
            )
    ...
    def _detect_outliers_elliptic(self, data: pd.DataFrame, contamination: float = 0.1) -> Dict[str, Any]:
        ...
        try:
            ...
            elliptic = EllipticEnvelope(contamination=contamination, random_state=self.random_state)
            ...
            for i in range(len(correlation_matrix.columns)):
                for j in range(i + 1, len(correlation_matrix.columns)):
                    ...
                    if abs(corr) > config.correlation_threshold:
                        high_correlations.append({"feature1": col1, "feature2": col2, "correlation": corr})
                        correlation_warnings.append(f"High correlation between '{col1}' and '{col2}': {corr:.3f}")
    ...
    def _validate_sample_weights(
        self, sample_weight: np.ndarray, data: pd.DataFrame, results: ValidationResults, config: ValidationConfig
    ) -> None:
                    ...
                    profile.update(
                        {
                            ...
                            "most_frequent_percentage": (
                                float(value_counts.iloc[0] / len(non_null_data) * 100) if len(value_counts) > 0 else 0
                            ),
    ...
    def _detect_data_drift(
        ...
        drift_summary = {
            ...
            "drift_percentage": (
                (features_with_drift / total_features_checked * 100) if total_features_checked > 0 else 0
            ),
    ...
    def _generate_recommendations(self, results: ValidationResults, config: ValidationConfig) -> None:
        ...
        if results.missing_value_analysis:
            high_missing_columns = [
                col for col, stats in results.missing_value_analysis["column_stats"].items() if stats["percentage"] > 20
            ]
        ...
        if skewed_features:
            recommendations.append(f"Consider transforming highly skewed features: {skewed_features}")
        ...
        if results.data_quality_score is not None:
            if results.data_quality_score < 60:
                ...
                recommendations.append("Data quality is moderate. Address key issues identified in the validation")

        results.recommendations = recommendations

    def generate_validation_report(
        self, results: ValidationResults, output_dir: Union[str, Path], include_plots: bool = True
    ) -> Dict[str, Path]:
    ...
    def _generate_validation_plots(self, results: ValidationResults, output_dir: Path) -> Dict[str, Path]:
        ...
        try:
            ...
            if (
                results.correlation_analysis
                and "correlation_matrix" in results.correlation_analysis
                and results.correlation_analysis["correlation_matrix"]
            ):
    ...
    def _plot_missing_values(self, missing_analysis: Dict[str, Any], output_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            for bar, pct in zip(bars, percentages):
                plt.text(
                    bar.get_width() + 0.5, bar.get_y() + bar.get_height() / 2, f"{pct:.1f}%", va="center", ha="left"
                )
    ...
    def _plot_data_quality_summary(self, results: ValidationResults, output_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            if results.feature_profiles:
                ...
                ax4.bar(quality_categories, quality_counts, color=["green", "orange", "red"])
...
def validate_dataset(
    data: pd.DataFrame, target_column: Optional[str] = None, sample_weight: Optional[np.ndarray] = None, **kwargs: Any
) -> ValidationResults:
...
def quick_data_check(
    data: pd.DataFrame, generate_report: bool = False, output_dir: Optional[Union[str, Path]] = None
) -> Dict[str, Any]:
    ...
    """
    config = ValidationConfig(generate_report=generate_report, include_visualizations=generate_report)
...
__all__ = ["ValidationConfig", "ValidationResults", "DataValidator", "validate_dataset", "quick_data_check"]
