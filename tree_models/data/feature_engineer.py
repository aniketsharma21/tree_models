...
class FeatureEngineer:
        ...
        try:
            with timed_operation("feature_engineering") as timing:
                ...
                results.new_features = [
                    col for col in results.data.columns 
                    if col not in results.original_features
                ]
            ...
            if col in data.columns:
                try:
                    ...
                    if date_col.dt.hour.notna().any():  # If time information is available
                        ...
                        data[f'{col}_is_business_hours'] = ((date_col.dt.hour >= 9) & (date_col.dt.hour <= 17)).astype(int)
    ...
    def _create_polynomial_interaction_features(
        ...
        if len(numeric_cols) > max_features_for_poly:
            logger.warning(f"Too many numeric features ({len(numeric_cols)}) for polynomial features. Using first {max_features_for_poly}.")
        ...
        if config.remove_correlated_features:
            ...
            if len(numeric_data.columns) > 1:
                ...
                high_corr_cols = [
                    ...
                    if any(upper_triangle[col] > config.correlation_threshold)
                ]
    ...
    def transform_new_data(
        ...
        try:
            ...
            for transformer_name, transformer in self.fitted_transformers_.items():
                if 'vectorizer' in transformer_name:
                    ...
                    if col_name in transformed_data.columns:
                        ...
                        text_df = pd.DataFrame(text_features.toarray(), columns=feature_names, index=transformed_data.index)
                    ...
                    if available_cols:
                        ...
                        poly_feature_names = [f'poly_{name}' for name in transformer.get_feature_names_out(available_cols)]
                        ...
                        if len(new_poly_names) > 0:
                            poly_df = pd.DataFrame(new_poly_features, columns=new_poly_names, index=transformed_data.index)
            ...
            handle_and_reraise(
                e, FeatureEngineeringError,
                "Failed to transform new data",
                error_code="TRANSFORM_NEW_DATA_FAILED"
            )
...
def create_time_series_features(
    ...
    **kwargs: Any
    ...
    time_config = {
        ...
        'rolling_windows': {
            col: {'window': 7, 'agg': 'mean'} for col in value_columns
        }
    }
    ...
    **kwargs: Any
    ...
    financial_config = {
        ...
        'create_ratios': [
            ('income', 'expenses'),
            ('assets', 'liabilities')
        ] if 'expenses' in data.columns else [],
        'create_bins': {col: 5 for col in amount_columns[:3]}  # Limit to first 3 columns
    }
